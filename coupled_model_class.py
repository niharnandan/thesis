...
class CoupledModel:
	...
	def __init__(self, values):
		...
		self.forcingtotal = forcing_total.forcing_total(forcing=forcing, alpha_doeclim=self.asc, l_project=False, begyear=self.mod_time[0], endyear=np.max(self.mod_time))
		doeclim_out = doeclimF.doeclimF(self.forcingtotal, self.mod_time, S=self.cs, kappa=self.ovd)
		self.temperatures = np.array((doeclim_out.loc[(doeclim_out["time"]>=1880) & (doeclim_out["time"]<=2008), "temp"]).tolist())
	...
	def coupled_model(forcingtotal, doeclim_out, temperatures, model, parameters):
		...
		if self.climate_sensitivity.value and self.ocean_vertical_diffusivity.value:
			doeclim_out = doeclimF.doeclimF(forcingtotal, self.mod_time, S=parameters[5], kappa=parameters[6])
	...
	def chain(self, deltat, N=10000):
		if self.aerosol_scaling.value:
			forcingtotal = forcing_total.forcing_total(forcing=forcing, alpha_doeclim=self.aerosol_scaling, l_project=False, begyear=mod_time[0], endyear=np.max(mod_time))
		if self.climate_sensitivity.value and self.ocean_vertical_diffusivity.value:
			doeclim_out = doeclimF.doeclimF(forcingtotal, self.mod_time, S=self.climate_sensitivity.value, kappa=ocean_vertical_diffusivity)
		...
		model = gmsl_model.gmsl_model(theta, temperatures, deltat) 	if self.alpha.value else []
		...
		for i in tqdm(range(N)):
			...
			if len(parameters) > 10: 
				self.coupled_model(forcingtotal, doeclim_out, temperatures, model, parameters)
			model = gmsl_model.gmsl_model(theta, temperatures, deltat) 	if self.alpha.value else []
...
'sigma_ar': 3, 'climate_sensitivity': 0, 'ocean_vertical_diffusivity': 0, 
'aerosol_scaling': 0, 'T_0': 0, 'sigma_T': 0, 'rho_T': 0}
...
'''
import subprocess
output = subprocess.getoutput("Rscript file")
'''
